...
public:
	Matrix(double*u, double*d, double*l, double *b, unsigned int len) {
	...
	double *thomas3() {
		...
		x[len - 1] = b[len - 1] * 1.0 / d[len - 1];
		for (int i = len - 2; i >= 0; i--)
			x[i] = (b[i] - u[i] * x[i + 1]) * 1.0 / d[i];
...
void printVector(char *name, double *x, unsigned int len, bool inRow=false) {
	if (inRow) {
		printf("%s=[\n", name);
		for (int i = 0; i < len - 1; i++)
			printf("%+.5e;\n", x[i]);
...
void matrixMultiVector(double **A, double *x, unsigned int len, bool i ) {
